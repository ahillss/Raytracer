...
vec3 render(vec3 ro,vec3 rd) {
    ...
    uint camerasStart=read1u(0u);
    //uint camerasNum=read1u(1u);
    uint pointLightsStart=read1u(2u);
    //uint pointLightsNum=read1u(3u);
    //uint spotLightsStart=read1u(4u);
    //uint spotLightsNum=read1u(5u);
    vec2 framesRange=uintBitsToFloat(read2u(6u));
    float frameLen=framesRange.y-framesRange.x;
    float curTime=mod(iTime*10.0,frameLen)+framesRange.x;
    
    uint cameraFrameSize=8u;
    uint pointLightFrameSize=9u;
    uint spotLightFrameSize=15u;
    
    uint camera0FramesStart=read1u(camerasStart+0u*2u+0u);
    uint camera0FramesNum=  read1u(camerasStart+0u*2u+1u);
    uint camera0FramesEnd=camera0FramesStart+camera0FramesNum*cameraFrameSize;

    uint pointLight0FramesStart=read1u(pointLightsStart+0u*2u+0u);
    uint pointLight0FramesNum=  read1u(pointLightsStart+0u*2u+1u);
    uint pointLight0FramesEnd=pointLight0FramesStart+pointLight0FramesNum*pointLightFrameSize;
    
    vec3 lightPos3=vec3(0.0);
    
    /*float start,num;
    searchKeyframe(pointLight0FramesStart,pointLightFrameSize,
                                curTime,start,num,frame);
    ...
    for(uint i=0u;i<pointLight0FramesNum;i++) {
        uint j=pointLight0FramesStart+i*pointLightFrameSize;

        float frame=uintBitsToFloat(read1u(j+0u));
        vec3 col=uint_to_vec4(read1u(j+1u)).rgb;
        float energy=uintBitsToFloat(read1u(j+2u));
        float dist=uintBitsToFloat(read1u(j+3u));
        float linAtten=uintBitsToFloat(read1u(j+4u));
        float quadAtten=uintBitsToFloat(read1u(j+5u));
        vec3 pos=uintBitsToFloat(read3u(j+6u));
        
        aaaa[i]=vec4(pos,frame);
        
    }
    
    float curTime2=mod(iTime,5.0)+0.0;
    vec4 bbbb[2];
    bbbb[0]=vec4(-1.0,0.0,0.0, 0.0);
    bbbb[1]=vec4(1.0,0.0,0.0, 5.0);
    float fff=(curTime-aaaa[0].w)/(aaaa[1].w-aaaa[0].w);
    lightPos3=mix(aaaa[0].xyz,aaaa[1].xyz,smoothstep(0.0,1.0,fff));
